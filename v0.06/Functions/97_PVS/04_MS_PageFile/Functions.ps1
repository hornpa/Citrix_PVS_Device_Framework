...
<#
    .SYNOPSIS
        Microsoft PageFile
	.Description
      	Setzt die PageFile anhand der hintelegten XML datei.
    .NOTES
		Author: 
         Patrik Horn
		Link:	
         www.hornpa.de
		History:
         2017-01-25 - Redesign Code, usage of function from Emin Atac - p0w3rsh3ll.wordpress.com  (PHo)
...
# Source: p0w3rsh3ll.wordpress.com, Author Emin Atac
...
Process {
    ...
    $CTX_PVS_PageFileDestination = $Settings_Global.Settings.Global.PageFileDestination
    $CTX_PVS_PageFileSize = $Settings_Global.Settings.Global.PageFileSize
        
    Set-PageFile -DriveLetter "C","D" -None

    Set-PageFile -DriveLetter $CTX_PVS_PageFileDestination -InitialSize $CTX_PVS_PageFileSize -MaximumSize $CTX_PVS_PageFileSize

    $Message =  "Die Einstellungen für $scriptName_Sub wurden gesetzt."

    Write-Log_hp -Path $LogPS -Value "$Message" -Component $scriptName_Sub -Severity 1
